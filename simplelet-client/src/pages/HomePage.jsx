...
export default function HomePage() {
  ...
  // Controlled form state for immediate input feedback
  ...
  // Debounced filters state used specifically for API queries and URL updates
  ...
  const { newListings } = useRealTimeListings(null, userId, debouncedFilters);

  // Query relies on debouncedFilters instead of immediate filters state
  ...
  // Debounce typed inputs (500ms delay) so typing doesn't trigger rapid re-renders
  useEffect(() => {
  ...
  // Sync URL search params only when debounced filters update
  useEffect(() => {
  ...
  const handleSearchSubmit = (e) => {
    e.preventDefault();
    // Force immediate sync without waiting for debounce timeout
    setDebouncedFilters(filters);
  };
      ...
      <div className="flex flex-wrap gap-2 mt-2">
        ...
        {similarTypes.map((type) => (
          <button
            ...
            type="button"
            onClick={() => {
              const updated = { ...filters, house_type: type };
              setFilters(updated);
              setDebouncedFilters(updated);
            }}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-3">
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setDebouncedFilters((prev) => ({ ...prev, house_type: e.target.value }));
                }}
                className="input"
              >
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  handleFilterChange(e);
                  setDebouncedFilters((prev) => ({ ...prev, sort_by: e.target.value }));
                }}
                className="input"
              >
